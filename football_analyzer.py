...
def fetch_schedule():
    ...
    for ev in events:
        ...
        kickoff = date_str.replace("T", " ").split(".")[0] if date_str else ""
        ...
        try:
            ...
            if h2h:
                home_odd = h2h["outcomes"][0]["price"]
                draw_odd = h2h["outcomes"][1]["price"]
                away_odd = h2h["outcomes"][2]["price"]
            if spread:
                asian_handicap = spread["name"]
            if totals:
                total_line = totals["name"]
        except Exception as e:
            print(f"❌ 抓取初盘失败: {e}")
    ...
    for m in data:
        ...
        if datetime.now() >= kickoff - timedelta(minutes=45):
            ...
            try:
                ...
                odds_data = odds_res.json()
                markets = odds_data.get("markets", [])
                h2h = next((mkt for mkt in markets if mkt["key"] == "h2h"), None)
                spread = next((mkt for mkt in markets if mkt["key"] == "spreads"), None)
                totals = next((mkt for mkt in markets if mkt["key"] == "totals"), None)
                if h2h:
                    m["临盘主胜"] = h2h["outcomes"][0]["price"]
                    m["临盘平"] = h2h["outcomes"][1]["price"]
                    m["临盘客胜"] = h2h["outcomes"][2]["price"]
                if spread:
                    m["临盘亚盘"] = spread["name"]
                if totals:
                    m["临盘大小球"] = totals["name"]
